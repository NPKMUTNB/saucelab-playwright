...
test.describe('SauceDemo E2E Test - MCP Generated (JavaScript)', () => {
  test('Complete purchase flow from login to order confirmation using MCP Playwright', async ({ page }) => {
    ...
    console.log(`✅ MCP JavaScript Test completed successfully with user data: ${firstName} ${lastName}, ZIP: ${zipCode}`);
  ...
  test('Login with different user types', async ({ page }) => {
    ...
    // Test with problem_user to verify different behaviors
    await page.locator('[data-test="username"]').fill('problem_user');
    await page.locator('[data-test="password"]').fill('secret_sauce');
    await page.locator('[data-test="login-button"]').click();
    
    // Should still reach inventory page
    await expect(page).toHaveURL(/.*inventory.html/);
    await expect(page.locator('.title')).toHaveText('Products');
    
    console.log('✅ Different user type login test completed');
  });
  
  test('Verify product information and pricing', async ({ page }) => {
    // Login first
    await page.goto('https://www.saucedemo.com/');
    await page.locator('[data-test="username"]').fill('standard_user');
    await page.locator('[data-test="password"]').fill('secret_sauce');
    ...
    // Verify Sauce Labs Backpack is visible and has correct price
    ...
    const backpackDescription = page.locator('.inventory_item_desc').first();
    await expect(backpackDescription).toContainText('carry.allTheThings()');
    ...
    console.log('✅ Product information verification completed');
  });
});